...
const TIMER_STORAGE_KEY = 'study-planner:timers'
...
function writeStoredTimers(timers) {
  if (typeof window === 'undefined') return
  window.localStorage.setItem(TIMER_STORAGE_KEY, JSON.stringify(timers))
}
...
export default function DailyPlan() {
  ...
  function refreshPlan() {
    fetch(`${API_BASE}/plan/today`)
      ...
      .then((data) => {
        setPlan(data)
        // Restore or initialize timer state for in-progress blocks without losing pause/reset state
        if (data && data.blocks) {
          const storedTimers = readStoredTimers()
          setTimers(() => {
            const next = {}
            data.blocks.forEach((block) => {
              if (block.status !== 'in-progress') return
    ...
    <section id="plan" className="px-4 sm:px-6 md:px-10 lg:px-16 py-6 sm:py-8 max-w-5xl mx-auto font-body">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between mb-6">
        <div>
          ...
          <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-linen">
            {plan.dateLabel}
          </h2>
        </div>
        <button
          onClick={() => setShowAddExtra(true)}
          className="w-full sm:w-auto rounded-full border border-marigold/40 bg-marigold/10 px-4 py-2 text-xs font-semibold text-marigold hover:bg-marigold hover:text-ink transition"
        >
          + Add Extra Subject
        </button>
      </div>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-linen/70 mt-1 break-words">
              ...
              {isInProgress && (
                <div className="rounded-xl bg-ink/90 border border-marigold/30 p-3 sm:p-4 flex flex-col gap-3 sm:flex-row items-stretch justify-between">
                  ...
                  <div className="flex flex-col sm:flex-row flex-wrap items-stretch gap-2 w-full sm:w-auto">
                    ...
                    <button
                      ...
                      className={[
                        'w-full sm:w-auto min-h-[42px] px-4 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition',
                        timerState.isPaused
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto min-h-[42px] px-3.5 rounded-xl bg-ink border border-linen/20 text-linen/70 hover:text-linen text-xs font-medium flex items-center justify-center gap-1"
              ...
              {isPending && (
                <div className="flex justify-end border-t border-linen/5 pt-3">
                  <button
                    ...
                    className="w-full min-h-[44px] px-6 rounded-xl bg-marigold text-ink font-bold text-xs hover:opacity-90 transition flex items-center justify-center gap-2 shadow-sm"
                  >
                    ▶ Start Study Session
                  </button>
                </div>
